...
| Флаг | Управляет | Тип |
|------|-----------|-----|
| `dealer_mode` | Вид: один дилер vs вся сеть | Global |
| `network_mode` | Дашборд: агрегация по сети | Global |
| `trade_in_enabled` | Видимость Trade-in UI | Per-dealer ready |
| `devtools_enabled` | Platform DevTools | Debug |
